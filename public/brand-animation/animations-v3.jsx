...
function Stage({
  ...
  // Presentation mode for a shipped, read-only embed (e.g. a marketing/login
  // page) — no scrubber, no drop-shadow "frame" around the canvas, just the
  // composition autoplaying edge-to-edge. The design tool's own preview/edit
  // session never sets this, so it keeps its scrubber.
  hideControls = false,
  children,
}) {
  ...
  React.useEffect(() => {
    ...
    const measure = () => {
      const barH = hideControls ? 0 : 44; // playback bar height, when shown
      const s = Math.min(
  ...
  return (
    ...
    >
      ...
      <div style={{
        ...
        <svg
          ...
          style={{
            ...
            flexShrink: 0,
            boxShadow: hideControls ? 'none' : '0 20px 60px rgba(0,0,0,0.4)',
            display: 'block',
          }}
        >
      ...
      {/* Playback bar — stacked below canvas, never overlapping. Omitted
          entirely in presentation mode rather than just visually hidden, so
          it can't be tabbed to or take up layout space. */}
  ...
  return (
    <React.Fragment>
      <Stage width={width} height={height} duration={derived.total} background={bg}
             autoplay={autoplay} loop={loop} playback={pb} hideControls={!!props.hideControls}>
